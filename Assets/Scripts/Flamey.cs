...
public class Flamey : MonoBehaviour, Hittable
{
    ...
    {
        Enemy en = Enemy.getClosestEnemy(transform.parent.position);
        Vector3 target = en.HitCenter.position;


        if (Vector2.Distance(transform.parent.position, target) > 7.5f) {
            GetComponent<Animator>().SetBool("Walking", false);
            return;
        }
        Vector3 directionAway = (transform.position - target).normalized;

        transform.parent.position += directionAway * TotemWalkSpeed * Time.deltaTime;
        transform.parent.position = new Vector3(Mathf.Clamp(transform.parent.position.x, -maxDistanceX, maxDistanceX), Mathf.Clamp(transform.parent.position.y, -maxDistanceY, maxDistanceY), 0);
        bool direction = transform.parent.position.x > target.x;
        transform.GetChild(2).GetComponent<SpriteRenderer>().flipX = direction;
    ...
    public void shoot()
    {
        ...
        ObjectPooling.Spawn(DefaultFlarePrefab.GetComponent<IPoolable>(), new float[] { FlameType, ShootingPoint.position.x, ShootingPoint.position.y });
    }
    ...
    public Flare InstantiateShot(List<string> except = null)
    {
        ...
        GameObject go = ObjectPooling.Spawn(DefaultFlarePrefab.GetComponent<IPoolable>(), new float[] { FlameType, ShootingPoint.position.x, ShootingPoint.position.y });
